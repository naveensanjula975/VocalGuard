    ...
    <nav className="relative px-4 py-4 mt-5 bg-white sm:px-8" aria-label="Main navigation">
      <div className="flex items-center justify-between mx-auto max-w-7xl">
        ...
        <div className="items-center hidden gap-12 md:flex">
          {navLinks.map((link) => (
            <Link key={link.to} to={link.to} className={LINK_BASE}>
              {link.label}
            </Link>
          ))}
          ...
          {user && (
            <div className="relative" ref={profileRef}>
              ...
              {isProfileOpen && (
                <div className="absolute right-0 z-50 w-48 py-1 mt-2 bg-white rounded-lg shadow-lg border border-gray-100" role="menu" aria-label="User menu">
      ...
      >
        <div className="flex flex-col gap-2 p-4">
          ...
          {user && (
            <>
              <Link to="/profile" className={MOBILE_LINK}>
                Profile Settings
              </Link>
              <button onClick={handleLogout} className="px-4 py-2 text-base font-medium text-left text-red-600 rounded-lg hover:bg-red-50">
                Logout
              </button>
            </>
          )}
